...
/**
 * Ultra-fast, zero-allocation event dispatcher with priority ordering and safe exception handling.
 */
public final class EventBus {
    ...
    /**
     * Registers all @EventListener annotated methods on the subscriber object.
     */
        ...
        for (Method method : subscriber.getClass().getDeclaredMethods()) {
            if (method.isAnnotationPresent(EventListener.class)) {
                ...
                if (paramTypes.length == 1 && Event.class.isAssignableFrom(paramTypes[0])) {
                    ...
                    // Keep list sorted by priority order (HIGHEST first)
                    invokers.sort(Comparator.comparingInt(inv -> inv.priority.getValue()));
                } else {
                    LOGGER.warn("Method {} in {} has @EventListener but invalid signature (must have exactly 1 parameter extending Event).",
    ...
    /**
     * Unregisters all listener methods on the given subscriber.
     */
